...
cd $LIB_DIR/nginx-$NGINX_VER && ./configure --prefix=/usr/share/nginx --pid-path=/run/nginx.pid -conf-path=/etc/nginx/nginx.conf --with-openssl=$LIB_DIR/openssl --with-cc-opt="-I$LIB_DIR/openssl/oqs/include" --with-ld-opt="-L$LIB_DIR/openssl/oqs/lib" && sed -i 's/libcrypto.a/libcrypto.a -loqs/g' objs/Makefile && make $MAKE_PARAM && make install || exit 1
